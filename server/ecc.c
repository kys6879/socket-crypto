...
static void ecc_native2bytes(uint8_t p_bytes[ECC_BYTES], const uint64_t p_native[NUM_ECC_DIGITS])
{
	...
	int oct = 8;
	int count = 0;
	for (i = 0; i < NUM_ECC_DIGITS; ++i) // 4
	{
		status = 8 * (NUM_ECC_DIGITS - 1 - i);  // 24 16 8 0
		uint8_t *key_address = p_bytes + status;
		uint8_t *p_digit = key_address;
		uint64_t p_temp[NUM_ECC_DIGITS];
		...
		//        for(j=0; j<8; j++ , count ++){
		//            p_temp[i] = p_native[i] >> oct*count;
		//            printf("p_digit[%d] : %02x\n", status+oct-j-1  ,  p_temp[i] );
		//        }
		count = 0;
...
{
	uint64_t l_private[NUM_ECC_DIGITS]; // 4
	EccPoint l_public;
	unsigned l_tries = 0;
	...
	p_publicKey[0] = 2 + (l_public.y[0] & 0x01); // 3
	return 1;
}
